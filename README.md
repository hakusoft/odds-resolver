...
構想・設計段階。配信基盤（S3 + CloudFront + OIDC）とサンプルデータでの TOP / 詳細 UI は動作中。
取得元は **地方競馬の対象サイト** に確定。取得層と夜間バッチの具体設計は
[docs/scraping-design.md](docs/scraping-design.md) を参照。取得の**インフラ方針**
（VPC 外 Lambda・送信元 IP 可変）は上記のとおり確定。

公開/非公開の最終方針（取得元の規約に関わる）は未確定。S3 に置くまでの収集設計は
これと独立に進める。
