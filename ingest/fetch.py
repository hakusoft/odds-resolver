...
#
#   日付    R  会場  T-10  T-60 T-240 T-480  exotic  T-10率
#   8/25   46   3     37    12    27    22       0     80%  ← 組合せ取得の前
#   8/26   46   3      4     8    19     7      43      9%
#   8/27   48   4      0     8    16     4      48      0%
#   8/28   36   3      0    20    26    14      36      0%
#   8/29   21   2     15    21    21    15      21     71%
#
# 原因は券種の数ではなく run() の構造（#139）。run() は _run_exotic を先に
# 見て、取れたらそこで return する（その理由は run() 側のコメント参照）。
# 1 起動 1 リクエストなので、**組合せを取った起動では単複が必ず落ちる**。
#
# **ただし常に潰れるわけではない（#141）。** 効くのは R 数ではなく
# **同時開催の会場数＝発走の密度**:
#
#   8/28  36R  船橋/笠松/園田（3 場）  発走間隔の中央値 10 分  → T-10 は 0
#   8/29  21R  帯広ば/佐賀（2 場）     発走間隔の中央値 20 分  → T-10 は 15
#
# 3 場開催だと発走が 10 分間隔で並び、あるレースの T-10 が別レースの締切間際と
# 重なる。組合せに枠を取られて T-10 が埋まらない。2 場だと間隔が 20 分に開き、
# 衝突が減って T-10 が残る。**8/28 は 36R でも 0 だった**ので、R 数だけでは
# 説明が付かない（かつてここに「実測 0 件」と書いたのは混雑日 4 日だけを
# 見た結論だった）。
#
...
#
# 混雑日に T-10 が欠けるのは**仕様であってバグではない**。逆に言えば、
# **2 場開催の日に T-10 が 0 なら、それは別の異常**なので調べる価値がある。
