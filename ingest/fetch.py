...
#
# **この値は混雑日の T-10 スロットを潰す。**
#
#   日付    R  会場  間隔  T-10  T-60 T-240 T-480  exotic  T-10率
#   8/25   46   4   10分    37    12    27    22       0     80%  ← 組合せ取得の前
#   8/26   46   4   15分     4     8    19     7      43      9%
#   8/27   48   4   15分     0     8    16     4      48      0%
#   8/28   36   3   10分     0    20    26    14      36      0%
#   8/29   21   2   20分    15    21    21    15      21     71%
#
# 原因は券種の数ではなく run() の構造（#139）。run() は _run_exotic を先に
# 見て、取れたらそこで return する（その理由は run() 側のコメント参照）。
# 1 起動 1 リクエストなので、**組合せを取った起動では単複が必ず落ちる**。
#
# **ただし常に潰れるわけではない（#141）。** 8/29 は 15 件取れている。
# かつてここに「構造的に潰す（実測 0 件）」と書いたが、それは組合せ取得を
# 始めてからの混雑日 3 日だけを見た結論だった。
#
# **どういう時に欠けるかは、まだ言い切れていない。** 分かっているのは:
#
#   - 組合せ取得が無い日は 80% 取れていた（8/25）。組合せが枠を食うのは確か
#   - 発走が疎な日は残る（8/29 は 2 場・間隔 20 分で 71%）
#   - **密度だけでは説明が付かない。** 8/25 は 4 場・間隔 10 分でも 80%
#     取れており、8/28 は 36R・3 場・間隔 10 分で 0。同じ密度で結果が違う
#
# 8/25 と 8/28 の差は組合せ取得の有無なので、「組合せ取得 × 混雑」の
# 組み合わせで決まる可能性が高いが、**日数が足りず確かめていない**。
# 断定を避けてここに事実だけ残す。
#
# 排他になっていることは実データで確認済み（exotic有 / うち T-10 も有）:
# 8/26 は 43R/1R、8/27 は 48R/0R、8/28 は 36R/0R。
#
# **#138 で 1 券種に減らしても 0 のまま**だった。かつてここに「2 券種で
# 2 分かかるため 37→4」と書いてあったが、それは原因の取り違えだった。
#
...
#
# 組合せ取得のある混雑日に T-10 が欠けるのは**仕様であってバグではない**。
# ただし上記のとおり条件は確定していないので、**T-10 の欠測を見ただけで
# 「いつもの仕様」と片付けないこと**。疎な日に 0 なら別の異常を疑う。
