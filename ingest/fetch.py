...
#
# **この値は T-10 スロットを圧迫する。度合いは取る券種の数で決まる（#143）。**
#
#   日付    R  会場  間隔  券種  T-10  exotic  T-10率
#   8/25   46   4   10分    0     37       0     80%   ← 組合せ取得の前
#   8/26   46   4   15分    2      4      43      9%
#   8/27   48   4   15分    2      0      48      0%
#   8/28   36   3   10分    2      0      36      0%
#   8/29   21   2   20分    1     15      21     71%
#   8/30   33   3   15分    1     23      33     70%
#   8/31   48   4   10分    1     32      48     67%   ← 8/27 と同条件
#
# **8/31 が決め手。** 48R・4 場・間隔 10 分は 8/27（T-10 は 0）と同条件だが、
# 組合せを 48 件取りながら T-10 は 32 件（67%）取れた。R 数・会場数・発走間隔
# では説明が付かず、**券種数だけが説明する**:
#
#   2 券種（8/26-28）   9% /  0% /  0%
#   1 券種（8/29-31）  71% / 70% / 67%
#   0 券種（8/25）     80%
#
# 境目は #138（三連複を外す）のデプロイ時刻（8/28 23:15 UTC = 8/29 8:15 JST）
# と一致する。仕組みは run() の構造（run() は _run_exotic を先に見て、取れたら
# そこで return する）。1 起動 1 リクエストなので組合せを取った起動では単複が
# 落ち、**T-10 の許容窓 2 分に対し 2 券種で 2 分かかると窓を使い切る**。
# 1 券種なら 1 分で済み、窓が半分残るので T-10 も取れる。
#
# **反証済みの仮説（同じ説を再び立てないため）:**
#
#   - 「発走の密度・会場数で決まる」→ 8/31（4 場・10 分）が 67% で反証
#   - 「run() の構造ゆえ構造的に 0」→ 1 券種の 3 日がいずれも 7 割で反証
#
# 後者は #139/#140 で書いたもので、**根拠にした 8/28 のデータが #138 の
# デプロイ前だった**（＝まだ 2 券種）。デプロイ時刻を確認せず「1 券種化後」と
# 誤認したのが起点。#136 が最初に書いた「2 券種で 2 分かかるため」は正しかった。
#
...
#
# 現在（1 券種）の期待値は **T-10 が 7 割前後**。これを下回る日が続いたら
# 券種数を確かめ、変えていないのに落ちているなら別の異常を疑う。
# 2 券種で 0 に近いのは**仕様であってバグではない**。
