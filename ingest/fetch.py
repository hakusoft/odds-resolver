...
#
# **この値は T-10 スロットを圧迫する。** 度合いは 2 つで決まる（#143 / #145）:
# **取る券種の数**と、**単位時間あたりのレース密度（R/h）**。
#
#   日付    R  会場 開催h  R/h  需要/h 券種  T-10率  T-15率
#   8/25   46   4    9.1  5.1   45.6    0     80%    100%   ← 組合せ取得の前
#   8/26   46   4   10.2  4.5   40.7    2      9%    100%
#   8/27   48   4   10.2  4.7   42.5    2      0%    100%
#   8/28   36   3   10.2  3.5   31.9    2      0%    100%
#   8/29   21   2    6.1  3.5   31.1    1     71%    100%
#   8/31   48   4    9.1  5.3   47.6    1     67%    100%
#   9/02   48   4   10.2  4.7   42.5    1     60%    100%
#   9/03   60   5   10.2  5.9   53.1    1     37%     98%
#   9/04   36   3    6.3  5.7   51.2    1     36%    100%
#   9/06   55   5    8.8  6.2   56.0    1     33%    100%
#
# **券種数**: 境目は #138（三連複を外す）のデプロイ時刻（8/29 8:15 JST）と
# 一致する。2 券種だと T-10 の許容窓 2 分を使い切り、ほぼ 0 に落ちる。
# 1 券種なら 1 分で済み窓が半分残る。#136 が最初に書いた説明が正しかった。
#
# **密度（#145）**: 1 券種でも 71% → 33% と落ちる。R 数でも会場数でもなく
# R/h が効く（9/04 は 36R と小規模だが開催 6.3h で高密度、36%）。
# 9 日の実測で **R/h と T-10率 の相関 r = -0.829**。
#
# 仕組み: 勝負どころは 9 スロット（T-45/30/20/15/10/8/6/4/2）あるので
# 必要な取得数は **R × 9 ÷ 開催時間**。1 起動 1 リクエストなので枠は毎時 60。
# 9/06 は需要 56.0 で**枠の 93%** を使い切っている。_pick は
# 「スロット超過 ÷ 許容窓」の比率が最大のレースを選ぶため、窓の広い
# T-45(15分)/T-30(10分)/T-20・T-15(5分) が優先され、**窓が最も狭い
# T-10 以降(2分)から溢れる**。設計どおりの挙動。
#
# **反証済みの仮説（同じ説を再び立てないため）:**
#
#   - 「run() の構造ゆえ構造的に 0」（#139/#140）→ 1 券種の日が 7 割で反証。
#     根拠にした 8/28 のデータが #138 のデプロイ前（＝まだ 2 券種）だった
...
#
#   - T-10 以内の観測は全レースに残る。T-10 が 0 だった 8/28 でも、36R
#     すべてが T-10 以内に 2〜4 点を持つ（28R が T-8/6/4/2、6R が T-6/4/2、
#     他 2R）。欠けるのは T-10 という**ラベル**であって、その時間帯の観測ではない
#   - **T-45 は全日 100%、T-15 もほぼ 100%**（9 日で欠けたのは 9/03 の 1 レース
#     のみ。その 1 件も前後の T-20/T-10 は取れている）。締切前の市場は
#     十分に捉えられている
#   - EDGE_SLOT_MINUTES は「T-10 ちょうど」ではなく**閾値**（_record_edges の
#     `minutes_to_post > EDGE_SLOT_MINUTES` を参照）。T-10 以内で最初に
#     取れた 1 点で記録されるので、T-8 でも T-6 でも成立する。#106 の
#     前向きログが 1 日平均 34 頭で途切れず貯まっているのがその証拠
#   - ここをずらすと単勝の乖離と時点がずれ、「どちらの歪みが大きいか」の
#     比較が成立しなくなる。それがこの取得の目的なので崩せない
#   - 取り逃しは closed_slots に明示的な欠測として残る
#
# 副次効果として、#138 で 1 券種に減らした分の余裕はベースライン層に回った
# （8/27→8/28 で T-240 が 16→26、T-480 が 4→14）。
#
# **監視は T-10 率で行わない（#145）。** 密度で 33〜71% を正常に上下するので
# 警報に使えない。かつてここに「1 券種なら 7 割前後」と書いたが、それは
# R/h 4 未満の低密度 3 日だけを見た値で、高密度日には当てはまらなかった。
#
# **見るべきは T-45 と T-15。** 密度によらず 100% を保っており、崩れたら
# 本物の異常（枠が足りないのではなく、取得そのものが止まっている）。
# T-10 が低いだけなら混雑した開催日というだけで、対処は要らない。
#
# 券種を増やしていないのに T-45/T-15 が落ちた時だけ調べる。
