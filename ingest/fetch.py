...
#
# **この値は T-10 スロットを構造的に潰す（実測 0 件）。**
#
#   日付    T-10  T-60  T-240  T-480
#   8/25     37    12     27     22   ← 組合せ取得の前
#   8/26      4     8     19      7
#   8/27      0     8     16      4
#   8/28      0    20     26     14   ← #138 で三連複を外した後
#
# 原因は券種の数ではなく run() の構造（#139）。run() は _run_exotic を先に
# 見て、取れたらそこで return する（その理由は run() 側のコメント参照）。
# 1 起動 1 リクエストなので、**組合せを取った起動では単複が必ず落ちる**。
# 組合せの窓 10 分と T-10 の許容窓 2 分が正面衝突するため、T-10 は埋まらない。
#
...
#
#   - T-10 以内の観測は全レースに残る。8/28 の 36R すべてが T-10 以内に
#     2〜4 点を持つ（28R が T-8/6/4/2、6R が T-6/4/2、他 2R）。消えたのは
#     T-10 という**ラベル**であって、その時間帯の観測ではない
