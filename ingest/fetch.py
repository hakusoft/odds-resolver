...
#
# **この値は T-10 スロットを潰す。** 許容窓 2 分に対し組合せ 2 券種で 2 分
# かかるため、実測で T-10 の取得が 37→4 に落ちた（8/25 と 8/26 の比較）。
# 承知の上で 10 のままにしている（#56 で判断）:
#
#   - 勝負どころの T-45/T-30/T-20/T-15 は無傷（46/46）。締切前の市場は
#     十分に captureできている
#   - ここを下げると単勝の乖離と時点がずれ、「どちらの歪みが大きいか」の
#     比較が成立しなくなる。それがこの取得の目的なので崩せない
#   - 取り逃しは closed_slots に明示的な欠測として残る
#
# T-10 が薄いのは**仕様であってバグではない**。
